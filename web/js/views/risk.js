...
// test, all from contract state at the current block.
...
export function mount(el, { query, setQuery }) {
  ...
  el.innerHTML = `
    <div class="page-head"><div><h1>Risk</h1><div class="sub">Liquidation exposure of every open position, recomputed from contract state each block.</div></div><span class="meta faint" id="block"></span></div>
    <div class="stack">
      ...
      <section class="panel"><div class="panel-head"><h2>By market</h2><span class="meta">Notional liquidated by an adverse move; shortfall is equity below zero at that price</span></div><div class="panel-body flush" id="table">${skeleton(8)}</div></section>
    </div>`;
  ...
  async function load() {
    ...
    $('kpis').innerHTML = [
      kpi({ label: 'Position notional', value: usd(t.total_notional), note: `${int(t.positions)} positions`, tip: 'Mark notional of every open position, long and short.' }),
      kpi({ label: 'At risk, 5% move', value: usd(t.notional_at_5pct), note: `${pct(num(t.notional_at_5pct) / num(t.total_notional) * 100, { digits: 1 })} of notional`, tip: 'Notional liquidated if each market moves 5% in its worse direction.' }),
      kpi({ label: 'At risk, 10% move', value: usd(t.notional_at_10pct), note: `${pct(num(t.notional_at_10pct) / num(t.total_notional) * 100, { digits: 1 })} of notional` }),
      kpi({ label: 'Shortfall, 10% move', value: `<span class="${num(t.shortfall_at_10pct) > 0 ? 'neg' : ''}">${usd(t.shortfall_at_10pct)}</span>`, note: 'potential bad debt', tip: 'Equity below zero at that price if positions are not liquidated first.' }),
      kpi({ label: 'Insurance funds', value: usd(t.insurance_total), note: t.insurance_coverage_at_10pct ? `${multiple(t.insurance_coverage_at_10pct)} the 10% shortfall` : '' }),
      kpi({ label: 'Book cover, 10% move', value: t.liquidity?.cover_at_10pct_pct ? atLeast(t.liquidity.complete) + multiple(t.liquidity.cover_at_10pct_pct) : '—', note: 'depth vs liquidation demand', tip: 'Resting order-book depth within the move compared with the notional that would be liquidated into it. "≥" marks a book deeper than the levels read.' })
    ].join('');
    ...
    $('table').innerHTML = table({ id: 'risk', columns: [
      ...
      { key: 'book', label: 'Book cover 10%', n: true, render: r => (r.liquidity?.cover_at_10pct?.min_pct ? atLeast(r.liquidity.cover_at_10pct.complete) + multiple(r.liquidity.cover_at_10pct.min_pct) : '—') },
  ...
  async function stress() {
    ...
    $('stress').innerHTML = `<div class="stat-grid">
        ...
        <div class="stat"><span>Share of open interest</span><span>${pct(r.liquidated.share_of_oi_pct, { digits: 1 })}</span></div><div class="stat"><span>Side hit</span><span>${sideTag(r.side)}</span></div>
