...
export function mount(el, { query, setQuery }) {
  ...
  async function load() {
    ...
    $('kpis').innerHTML = [
      kpi({ label: 'Position notional', value: usd(t.total_notional), note: `${int(t.positions)} positions`, tip: 'Mark notional of every open position, long and short (open interest counts one side).' }),
      ...
      kpi({ label: 'At risk, 10% move', value: usd(t.notional_at_10pct), note: `${pct(num(t.notional_at_10pct) / num(t.total_notional) * 100, { digits: 1 })} of notional · market-wide ${esc(t.direction_at_10pct ?? '')}` }),
      kpi({ label: 'Shortfall, 10% move', value: `<span class="${num(t.shortfall_at_10pct) > 0 ? 'neg' : ''}">${usd(t.shortfall_at_10pct)}</span>`, note: 'potential bad debt', tip: 'Equity below zero after a market-wide 10% move in the worse direction, if no liquidation executes first.' }),
