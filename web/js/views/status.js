...
import { table, skeleton, empty } from '../ui.js';

const dot = s => `<span class="status-dot ${s}"></span>`;

export function mount(el) {
  let alive = true;
  el.innerHTML = `<div class="page-head"><div><h1>Data &amp; methodology</h1><div class="sub">How PerplScope gets its numbers, and live proof that they match the contract.</div></div></div>
    <div class="stack">
      ...
      <section class="panel"><div class="panel-head"><h2>Definitions</h2></div><div class="doc">
        ...
        <h3>Fees</h3>Maker plus taker fees charged on fills. The exchange splits every fee into an insurance-fund part and a protocol part (<code>insFeeCNS + protFeeCNS</code> on the position event equals the fill fee, checked on every trade). Builder fees are reported separately.
        ...
        <h3>Realized and net PnL</h3>Realized PnL is <code>deltaPnl + funding</code> reported by decrease, close, flip, liquidation and deleveraging events. Net PnL subtracts the fees the account paid on its fills.
        ...
        <h3>Freshness</h3>Only finalized blocks are indexed, so no number is ever rolled back. The node signals each finalized block (execution-events sidecar or WebSocket), the range is read with <code>eth_getLogs</code> and committed to ClickHouse, then pushed to this page over server-sent events.
      </div></section>
    </div>`;
  ...
  async function load() {
    ...
    $('pipeline').innerHTML = [
      ...
      `<div class="stage"><h3>${dot(h.snapshot?.status === 'fresh' ? 'ok' : 'warn')}Contract state</h3><div class="v">#${int(h.snapshot?.block)}</div><div class="d">${esc(h.snapshot?.status ?? '')} · ${int(h.collector?.polls)} polls · ${int(h.collector?.rpc_requests)} RPC requests</div><div class="d">${int(feed?.sse_clients)} live viewers</div></div>`
    ].join('');
    ...
    if (!integ || !integ.complete) $('integrity').innerHTML = empty(integ?.pending ? 'Waiting for the index to reach the contract snapshot block.' : 'Available once the full history is indexed (the running sums need every event since deployment).');
      ...
      { key: 'm', label: 'Market', render: x => esc(x.symbol) },
    ...
    $('checks').innerHTML = `<div class="stat-grid" style="grid-template-columns:1fr">
      ...
      <div class="stat"><span>${dot('ok')} Position PnL recomputed and compared with getPositionsV2</span><span>${(v?.pnl_agreement ?? []).reduce((a, x) => a + (x.agree ?? 0), 0)} / ${(v?.pnl_agreement ?? []).reduce((a, x) => a + (x.checked ?? 0), 0)} agree</span></div>
    </div>`;
