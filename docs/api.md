...
| `/api/v1/liquidations` | `limit` ≤ 500, `market`, `window`, `format=csv` | Latest liquidations and deleverages (inside `window` when given, with the window's `largest`), and `last_24h` totals |
